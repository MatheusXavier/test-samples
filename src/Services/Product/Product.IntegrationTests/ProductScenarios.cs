...
namespace Product.IntegrationTests
{
    public class ProductScenarios : ProductScenariosBase
    {
        ...
        public async Task Get_product_by_id_and_response_status_code_notfound()
        {
            ...
            // Act
            HttpResponseMessage response = await httpClient.GetAsync($"api/v1/products/{Guid.NewGuid()}");
        ...
        public async Task Get_product_by_id_and_response_status_code_ok()
        {
            ...
            // Act
            HttpResponseMessage response = await httpClient.GetAsync($"api/v1/products/{product.Id}");

            // Assert
            response.StatusCode.Should().Be(HttpStatusCode.OK);

            API.Domain.Entities.Product? content = await GetRequestContent<API.Domain.Entities.Product>(response);
        ...
        public async Task Post_create_a_new_product_and_response_status_code_ok()
        {
            ...
            // Act
            HttpResponseMessage response = await httpClient.PostAsync("api/v1/products", requestContent);
            ...
            API.Domain.Entities.Product? dbProduct = await server.ProductContext.Products.FindAsync(content?.Id);
        ...
        public async Task Put_update_product_and_response_status_code_notfound()
        {
            ...
            UpdateProductViewModel productToUpdated = new(Guid.NewGuid(), "Updated name", 1.0);
            StringContent requestContent = BuildRequestContent(productToUpdated);

            // Act
            HttpResponseMessage response = await httpClient.PutAsync("api/v1/products", requestContent);
        ...
        [Fact]
        public async Task Put_update_product_and_response_status_code_badrequest()
        {
            // Arrange
            using ProductTestServer server = CreateServer();
            using HttpClient httpClient = server.CreateClient();
            ...
            UpdateProductViewModel productToUpdated = new(product.Id, "", 0);
            StringContent requestContent = BuildRequestContent(productToUpdated);
            ...
            content.Should().Be("'name' cannot be null or empty. (Parameter 'name')");
        }
        ...
        public async Task Put_update_product_and_response_status_code_ok()
        {
            ...
            UpdateProductViewModel productToUpdated = new(product.Id, "Updated name (1)", 100.34);
            StringContent requestContent = BuildRequestContent(productToUpdated);
